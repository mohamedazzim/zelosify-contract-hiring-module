...
export async function rejectProfile(req: Request, res: Response) {
  try {
    ...
    // 2. Enforce state transition rules: Cannot reject an already shortlisted profile
    if (profile.status === ProfileStatus.SHORTLISTED) {
      return res.status(409).json({
        status: "error",
        message: "Cannot reject a candidate that has already been shortlisted. Shortlist must not be silently overwritten.",
      });
    }
    ...
    return res.status(500).json({
      status: "error",
      message: "Internal server error rejecting profile",
    });
  }
}
